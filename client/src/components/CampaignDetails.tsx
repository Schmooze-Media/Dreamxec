  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: open ? "5px 5px 0 #FF7F00" : "4px 4px 0 #003366",
      }}
    >
      <button
      ...
      {open && (
        <div className="px-4 sm:px-5 pb-4 pt-2 border-t-2 border-dashed border-dreamxec-navy/30 bg-amber-50">
          <p className="text-dreamxec-navy/85 text-xs sm:text-sm md:text-base leading-relaxed font-medium">
            {faq.answer}
          </p>
        </div>
      )}
...
function NoDescription() {
  return (
    <div className="text-center py-10 px-4 border-2 border-dashed border-dreamxec-navy/30 rounded-none bg-amber-50">
      <p className="text-dreamxec-navy/60 font-black uppercase tracking-widest text-sm">
        No description available
      </p>
      <p className="text-dreamxec-navy/40 text-xs mt-1">
        Campaign details will be updated soon.
      </p>
    </div>
  );
}
...
export default function CampaignDetails({
  ...
  const refreshCampaign = async () => {
    ...
    try {
      const er = await axios.get(
        `${API_BASE}/campaigns/${res.data.userProject.id}/endorsements`,
        { headers: token ? { Authorization: `Bearer ${token}` } : {} },
      );
      setEndorseCount(er.data.count ?? 0);
      setEndorsed(er.data.endorsedByMe ?? false);
    } catch {
      /* non-fatal */
    }
  };
  ...
  useEffect(() => {
    if (campaign) addRecentCampaign(campaign);
  }, [campaign]);
  ...
  useEffect(() => {
    const checkWishlistStatus = async () => {
      const isDonorUser =
        currentUser?.role === "donor" || currentUser?.role === "DONOR";
  ...
  const handleEndorse = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_BASE}/campaigns/${campaign!.id}/endorse`,
        {},
        { headers: { Authorization: `Bearer ${token}` } },
      );
      setEndorsed(res.data.endorsed);
      setEndorseCount(res.data.count ?? endorseCount);
    } catch {
      alert("Failed to endorse. Please try again.");
    } finally {
    ...
    return (
      <div className="min-h-screen bg-dreamxec-cream flex items-center justify-center">
        ...
        >
          ...
          <p className="font-black uppercase tracking-widest text-dreamxec-navy text-sm">
            Loading campaign...
          </p>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-dreamxec-cream flex items-center justify-center px-4">
        ...
        >
          ...
          <h2 className="text-xl font-black text-dreamxec-navy uppercase mb-2">
            {error ? "Error" : "Not Found"}
          </h2>
          <p className="text-dreamxec-navy/70 text-sm mb-6">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8 pb-24 sm:pb-28 lg:pb-12">
          ...
          ════════════════════════════════ */}
            <div className="w-full flex-1 min-w-0 space-y-5 sm:space-y-6">
                    ...
                    {isDonor && (
                      ...
                      >
                        <svg
                          ...
                          stroke={isWishlisted ? "#ef4444" : "#003366"}
                          viewBox="0 0 24 24"
                          strokeWidth={2}
                        >
                          <path
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  {[
                    { emoji: "🏛️", label: campaign.club?.college },
                    {
                      ...
                      onClick: campaign.club?.id
                        ? () => navigate(`/clubs/${campaign.club?.id}`)
                        : undefined,
                    },
                    ...
                    >
                      ...
                      <span className="truncate max-w-[120px] sm:max-w-none text-dreamxec-navy">
                        {chip.label}
                      </span>
                    </div>
                  ))}
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {(
                  ["about", "presentation", "video", "media", "faqs"] as const
                ).map((tab) => (
                  <TabBtn
                    key={tab}
                    label={tab}
              ...
              {activeTab === "media" && (
                <NeoCard className="p-4 sm:p-5 md:p-6" accentColor="#003366">
                  ...
                  campaign.campaignMedia.length > 0 ? (
                    ...
                    <p className="text-dreamxec-navy/60 font-bold text-sm uppercase tracking-wide">
                  ...
                  {(() => {
                    ...
                    const renderDocument = (fileUrl: string) => {
                        ...
                        return (
                          <div className="w-full h-full flex items-center justify-center p-4 bg-gray-50 overflow-y-auto">
                            <img
                              src={fileUrl}
                              alt="Presentation"
                              className="max-w-full h-auto object-contain shadow-md border-2 border-dreamxec-navy"
                            />
                          </div>
                        );
                      }
                      ...
                      if (lowerUrl.endsWith(".pdf") && isCloudinary) {
                        ...
                        return (
                          <div
                            className="relative w-full h-full group bg-gray-100 flex items-center justify-center cursor-pointer overflow-hidden"
                          ...
                          >
                            ...
                            <div className="absolute inset-0 bg-dreamxec-navy/30 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all backdrop-blur-sm">
                              <span className="p-4 bg-white rounded-full shadow-xl mb-3 text-2xl">
                                📄
                              </span>
                      ...
                      if (
                        ...
                        return (
                          <iframe
                            src={msUrl}
                            className="w-full h-full border-none bg-white"
                            title="Office Presentation Deck"
                          />
                        );
                      }
              ...
              >
                ...
                <p className="mt-4 pt-4 text-[10px] sm:text-xs text-dreamxec-navy/50 font-bold uppercase tracking-wide border-t-2 border-dashed border-dreamxec-navy/20">
                  * Timeline shows phased fund utilization across execution
                  milestones
                </p>
              </NeoCard>
          ...
          ════════════════════════════════ */}
            <div className="w-full lg:w-[360px] xl:w-[400px] flex-shrink-0 space-y-4 sm:space-y-5 lg:sticky lg:top-24 self-start">
              <NeoCard className="p-4 sm:p-5 md:p-6" accentColor="#FF7F00">
                ...
                <div className="mb-4 sm:mb-5 pb-4 sm:pb-5 border-b-2 border-dreamxec-navy/20">
                  ...
                  <p className="text-sm sm:text-base text-dreamxec-navy/60 font-bold mt-1">
                    raised of{" "}
                    <span className="text-dreamxec-navy">
                ...
                <div className="mb-4 sm:mb-5">
                  <div
                    className="w-full h-5 sm:h-6 bg-gray-100 overflow-hidden"
                ...
                <div className="mb-4 sm:mb-5">
                  ...
                  >
                    <p className="text-xl sm:text-2xl font-black text-dreamxec-navy">
                      ₹{remainingAmount.toLocaleString()}
                    </p>
                    <p className="text-[10px] sm:text-xs text-dreamxec-navy/60 font-black uppercase tracking-widest mt-0.5">
                      Remaining
                    </p>
                  </div>
                </div>
                ...
                {/* Performance Score */}
                <div
                  className="mb-4 sm:mb-5 p-3 sm:p-4"
                  style={{
                    border: "2px dashed #FF7F00",
                    background: "#fff7ed",
                  }}
                >
                  <p className="text-xs font-black text-dreamxec-navy uppercase tracking-wide mb-2">
                    Performance Score
                  </p>
                  <div className="flex items-center gap-0.5 sm:gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <svg
                        ...
                        viewBox="0 0 24 24"
                        fill={
                          (campaign.rating ?? 5) >= star ? "#FF7F00" : "none"
                        }
                        stroke="#FF7F00"
                        strokeWidth="2"
                        className="w-4 h-4 sm:w-5 sm:h-5"
                    ...
                    <span className="ml-2 font-black text-dreamxec-navy text-sm">
                      {(campaign.rating ?? 5).toFixed(1)} / 5
                    </span>
                  </div>
                  <p className="text-[10px] text-dreamxec-navy/50 font-bold mt-1">
                    Based on milestone performance
                  </p>
                </div>
                ...
                {campaign.status === "approved" && (
                  <button
                    ...
                    style={{
                      background:
                        "linear-gradient(135deg, #FF7F00 0%, #ef4444 100%)",
                      border: "3px solid #003366",
                      boxShadow: "5px 5px 0 #003366",
                    }}
                  >
                ...
                <div className="mt-4 sm:mt-5 pt-4 sm:pt-5 border-t-2 border-dreamxec-navy/20">
                  <p className="text-[10px] sm:text-xs font-black text-dreamxec-navy uppercase tracking-widest mb-2 sm:mb-3">
                    Share Campaign
                  </p>
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      ...
                      },
                      {
                        ...
                        action: () =>
                          window.open(
                            `https://twitter.com/intent/tweet?text=${encodeURIComponent("Check out this amazing campaign!")}&url=${encodeURIComponent(window.location.href)}`,
                            "_blank",
                          ),
                      },
                      {
                        ...
                        action: () =>
                          window.open(
                            `https://api.whatsapp.com/send?text=${encodeURIComponent(`Check out "${campaign.title}" on DreamXec! `)}${encodeURIComponent(window.location.href)}`,
                            "_blank",
                          ),
                      },
              ...
              {/* ── Alumni Endorsement Button ── */} {/* ← ADD THIS BLOCK */}
              {campaign.status === "approved" && (
                <>
                  {isAlumni ? (
                    /* Alumni: active endorse button */
                    <button
                      onClick={handleEndorse}
                      disabled={endorseLoading}
                      className="w-full py-3 font-black uppercase tracking-widest text-sm flex items-center justify-center gap-2 transition-all active:translate-x-[2px] active:translate-y-[2px]"
                      style={{
                        background: endorsed ? "#f0fdf4" : "#fff",
                        color: endorsed ? "#0B9C2C" : "#003366",
                        border: `3px solid ${endorsed ? "#0B9C2C" : "#003366"}`,
                        boxShadow: `4px 4px 0 ${endorsed ? "#003366" : "#FF7F00"}`,
                        opacity: endorseLoading ? 0.7 : 1,
                      }}
                    >
                      <svg
                        className="w-4 h-4"
                        fill={endorsed ? "#0B9C2C" : "none"}
                        stroke="currentColor"
                        strokeWidth={2.5}
                        viewBox="0 0 24 24"
                      >
                    ...
                    /* Non-alumni: locked teaser */
                    <div
                      className="w-full py-3 flex items-center justify-center gap-2"
                      style={{
                        background: "#f8f8f8",
                        border: "3px dashed #003366/30",
                      }}
                    >
                      <svg
                        className="w-4 h-4 text-[#003366]/30"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth={2}
                        viewBox="0 0 24 24"
                      ...
                      <span className="text-xs font-black text-[#003366]/40 uppercase tracking-widest">
                        Alumni Endorsement
                      </span>
                      <span
              ...
              {/* Share */}
              <div className="mt-4 sm:mt-5 pt-4 sm:pt-5 border-t-2 border-dreamxec-navy/20"></div>
              {/* Team Members */}
                    ...
                    <div className="space-y-3">
                      {campaign.teamMembers.map((member, index) => (
                        ...
                        >
                          <img
                            src={
                              member.image || "https://via.placeholder.com/100"
                            }
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="font-black text-dreamxec-navy text-xs sm:text-sm uppercase truncate">
                              {member.name}
                            </p>
                            <p className="text-[10px] sm:text-xs text-dreamxec-navy/60 font-medium truncate">
                              {member.role}
                            </p>
                          </div>
                        </div>

...
export default function ProfileSetup() {
  ...
  const [isAlumniEligible, setIsAlumniEligible] = useState(false); // ← ADD
  const [isMentorEligible, setIsMentorEligible] = useState(false); // ← ADD
  const [showActivationCards, setShowActivationCards] = useState(false); // ← ADD
  ...
  const [dInstitution, setDInstitution] = useState(""); // Step 2
  const [dGradYear, setDGradYear] = useState(""); // Step 2
  ...
  const [dExpertiseSkills, setDExpertiseSkills] = useState<string[]>([]); // Step 3
  const [dOpenToConnect, setDOpenToConnect] = useState(false); // Step 3
      ...
      try {
        ...
        if (res.status === "success" && res.data) {
          ...
          if (r === "DONOR") {
            ...
            setDPan(p.panNumber || "");
            setDInstitution(p.institution || ""); // ← ADD
            setDGradYear(p.gradYear || ""); // ← ADD
            ...
            setDExpertiseSkills(p.expertiseSkills || []); // ← ADD
            setDOpenToConnect(p.openToConnect || false); // ← ADD
          } else {
      ...
      return (
        {
          ...
          1: {
            panNumber: dPan,
            institution: dInstitution || undefined,
            gradYear: dGradYear || undefined,
          },
          2: {
            education: dEducation,
            occupation: dOccupation || undefined,
            address: dAddress,
            expertiseSkills: dExpertiseSkills.length
              ? dExpertiseSkills
              : undefined,
            openToConnect: dOpenToConnect || undefined,
          },
          3: {
  ...
  const getDashboardRoute = (r: Role): string => {
    switch (r) {
      case "DONOR":
      ...
      case "STUDENT_PRESIDENT":
        return "/president";
      case "ALUMNI":
        return "/donor/dashboard"; // no dedicated route yet
      case "MENTOR":
        return "/donor/dashboard"; // no dedicated route yet
      default:
        return "/dashboard"; // student, admin
    }
  };
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        // ── Capture eligibility flags from final-step response ──
        if (isLast && role === "DONOR") {
          if (res.data.isAlumniEligible) setIsAlumniEligible(true);
          if (res.data.isMentorEligible) setIsMentorEligible(true);
        }
      }
    } catch (err: any) {
    ...
    if (isLast) {
      ...
      if (role === "DONOR" && (isAlumniEligible || isMentorEligible)) {
        // ── Show activation cards instead of immediate redirect ──
        setShowActivationCards(true);
      } else {
        setTimeout(() => navigate(getDashboardRoute(role)), 1500);
      }
    } else {
      setStep((s) => s + 1);
    }
  ...
  // ── Activation Cards (post-setup screen) ──
  if (showActivationCards) {
    return (
      <div className="min-h-screen bg-[#FFFBF3] flex items-center justify-center py-10 px-4">
        <div className="w-full max-w-2xl">
          {/* Header */}
          <div className="text-center mb-8">
            <div className="text-5xl mb-3">🎉</div>
            <h1 className="text-3xl font-black text-[#003366] uppercase tracking-tight mb-2">
              You have unlocked new roles on DreamXec
            </h1>
            <p className="text-[#003366]/60 font-bold text-sm">
              Your profile qualifies you for the following — activate now or
          ...
          <div className="space-y-5">
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h2 className="text-lg font-black text-[#003366] uppercase tracking-tight mb-1">
                      Alumni
                    </h2>
                    <p className="text-sm font-medium text-[#003366]/70 mb-4">
                      Join our alumni network, mentor student innovators, and
                      get recognised on DreamXec.
                    </p>
                    <div className="flex gap-3 flex-wrap">
                      <button
                        onClick={async () => {
                          try {
                            ...
                            setSuccessMsg("✅ Alumni role activated!");
                          } catch {
                            setError(
                              "Failed to activate Alumni role. Please try again.",
                            );
                          }
                        }}
                        className="px-5 py-2.5 text-sm font-black text-white bg-[#003366] uppercase tracking-wide"
                        style={{
                          border: "3px solid #003366",
                      ...
                      <button
                        ...
                        className="px-5 py-2.5 text-sm font-black text-[#003366] bg-white uppercase tracking-wide"
                        style={{ border: "3px solid #003366" }}
                      >
                        Maybe Later
                      </button>
                    </div>
                    {successMsg && successMsg.includes("Alumni") && (
                      <p className="mt-3 text-sm font-bold text-green-700">
                        ✅ Alumni role activated!
                      </p>
                    )}
                  </div>
                </div>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-medium text-[#003366]/70 mb-4">
                      Apply to become a verified mentor and guide student
                      innovators on their journey.
                    </p>
                    <div className="flex gap-3 flex-wrap">
                      <button
                        onClick={() =>
                          navigate("/donor/dashboard?section=mentorship")
                        }
                        className="px-5 py-2.5 text-sm font-black text-white bg-[#0B9C2C] uppercase tracking-wide"
                        style={{
                          border: "3px solid #003366",
            ...
            {/* ── Always visible: Go to Dashboard ── */}
            <div className="text-center pt-2">
              <button
                onClick={() => navigate("/donor/dashboard")}
                className="px-6 py-3 text-sm font-black text-[#003366] bg-[#FFFBF3] uppercase tracking-wide"
                style={{
                  border: "3px solid #003366",
                  boxShadow: "3px 3px 0 #FF7F00",
                }}
                  ...
                  <>
                    ...
                    <div className="flex items-start gap-3 mt-1">
                      <input
                        ...
                        className="w-4 h-4 mt-0.5 accent-[#003366]"
                      />
                      <label

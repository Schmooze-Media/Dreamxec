...
function computeEligibility(donor) {
  ...
  // Alumni check — normalize institution (lowercase, strip hyphens/dots)
  const rawInstitution = donor.institution ?? "";
  const normalizedInstitution = rawInstitution
    .toLowerCase()
    .replace(/[-."]/g, "")
    .trim();
  ...
  // Both conditions must be true — either alone is insufficient (Gap 4)
  const isAlumniEligible = hasInstitution && hasGraduated;

  // Mentor check — soft signal only (Gap 5)
  // openToConnect = true only surfaces the Mentor Application sidebar
  const isMentorEligible = donor.openToConnect === true;

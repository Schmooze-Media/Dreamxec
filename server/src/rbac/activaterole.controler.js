...
exports.activateRole = catchAsync(async (req, res, next) => {
  ...
  if (role === "MENTOR") {
    ...
    // Inject authenticated user's email & name to ensure they match the logged-in account ──
    const applicationPayload = {
      ...mentorFormData,
      name: mentorFormData.name || req.user.name,
      email: req.user.email, // Always override with authenticated user's email
    };
